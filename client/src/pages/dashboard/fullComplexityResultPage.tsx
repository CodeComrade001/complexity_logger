...
export default function ComplexityResultPage() {
  ...
  const [fileComplexityResult, setFileComplexityResult] = useState<
  ...
  console.log("Turbo Log  ~ ComplexityResultPage ~ jobId:", jobId);
  ...
  useEffect(() => {
    ...
    const storedCodeAnalysis = fetchSession<FileComplexityData[]>(
      `compiler-job-${jobId}`
    );

    console.log(
      "Turbo Log ~ ComplexityResultPage ~ storedCodeAnalysis:",
      storedCodeAnalysis
    );

    if (storedCodeAnalysis?.length === 0) {
      setFileComplexityResult(null);
      return;
    }
   ...
   *         └── ComplexityReport
   *
   * Convert all reports from all files into one array
   * that the UI can work with.
   */
  const allFunctions = useMemo<UIFunction[]>(() => {
    if (!fileComplexityResult?.length) {
      return [];
    }

    return fileComplexityResult.flatMap((fileResult) => {
      if (!fileResult.success || !fileResult.result) {
        return [];
      }

      const report = fileResult.result;

      const fileName = report.nameOfFile || fileResult.fileName;
  ...
  const fileNames = [
    "all",
    ...new Set(allFunctions.map((fn) => fn.fileName)),
  ];
  ...
  const filteredFunctions = allFunctions.filter((fn) => {
    ...
    const matchesSearch =
      ...
      fn.fileName.toLowerCase().includes(query);

    const matchesRisk =
      filterRisk === "ALL" || fn.riskLevel === filterRisk;

    const matchesFile =
      selectedFile === "all" || fn.fileName === selectedFile;
  ...
  const summary = useMemo(() => {
    return {
      ...
      critical: filteredFunctions.filter(
        (fn) => fn.riskLevel === "CRITICAL"
      ).length,

      high: filteredFunctions.filter(
        (fn) => fn.riskLevel === "HIGH"
      ).length,

      medium: filteredFunctions.filter(
        (fn) => fn.riskLevel === "MEDIUM"
      ).length,

      low: filteredFunctions.filter(
        (fn) => fn.riskLevel === "LOW"
      ).length,

      avgScore: filteredFunctions.length
        ? Math.round(
          filteredFunctions.reduce(
            (sum, fn) => sum + fn.totalScore,
            0
      ...
      <div className="w-full mx-auto p-6 space-y-8 bg-background min-h-screen font-sans">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Deep dive into your codebase performance metrics.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
          {[
            ...
            <div
              key={i}
              className="glass-panel p-4 rounded-xl shadow-sm border-card-border"
            >
        ...
        <div className="overflow-auto max-h-[500px]">
              ...
              {paginatedFunctions.map((fn, index) => {
                ...
                return (
                  <div
                    key={`${fn.id}_${index}`}
                    className={cn(
                      "glass-panel flex flex-col rounded-xl overflow-hidden border-t-4 transition-all hover-elevate",
                      styles?.border
                    )}
                  >
                    <div className="p-5 space-y-4">
                      ...
                      <button
                        onClick={() =>
                          setActiveReasonId(
                            activeReasonId === fn.id ? null : fn.id
                          )
                        }
                      ...
                      >
                        ...
                        {activeReasonId === fn.id
                          ? "Hide Details"
                          : `View ${fn.reasons.length} Analysis Reasons`}
                      </button>
                      ...
                      {activeReasonId === fn.id && (
                        <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                          {fn.reasons.map((r, i) => (
                            <div
                              key={i}
                              className="p-3 bg-muted/30 rounded-lg border border-border text-[11px] leading-snug"
                            >
                        ...
                        <div className="flex-1 min-w-0 space-y-2">
                          <div className="flex items-start justify-between gap-2">
                            <div>
                              ...
                              <div className="text-xs text-muted">
                                {fn.fileName} · Lines {fn.startLine}–
                                {fn.endLine}
                              </div>
                            </div>
                            ...
                            <div
                              className={cn(
                                ...
                                getRiskBadgeClass(fn.riskLevel)
                              )}
                            >
                          ...
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="text-xs font-mono px-2 py-0.5 border rounded">
                              Time:{" "}
                              {fn.timeComplexity.notation || "Null"}
                            </span>

                            <span className="text-xs font-mono px-2 py-0.5 border rounded">
                              Space:{" "}
                              {fn.spaceComplexity.notation || "Null"}
                            </span>
                          ...
                          {activeReasonId === fn.id && (
                            <div className="space-y-2 pt-2">
                              {fn.reasons.map(
                                (reason: ComplexityReason, i: number) => (
                                  <div
                                    key={i}
              ...
              <table className="w-full text-sm">
                <thead className="panel-muted">
                  <tr className="text-left">
                    ...
                    <th className="p-3 font-semibold min-w-20">
                      Lines
                    </th>

                    <th className="p-3 font-semibold min-w-[100px]">
                      Time
                    </th>
                    ...
                    <th className="p-3 font-semibold min-w-20 text-center">
                      Score
                    </th>
                  ...
                  {paginatedFunctions.map((fn, index) => (
                    <>
                      <tr
                        key={`${fn.id}_${index}`}
                        className={cn(
                          "border-t border-l-4 hover:bg-muted",
                          getRiskClass(fn.riskLevel)
                        )}
                      >
                        ...
                        <td className="p-3 text-xs">{fn.fileName}</td>
          ...
          {filteredFunctions.length === 0 && (
            <div className="glass-panel py-20 text-center rounded-2xl">
              ...
              <p className="text-sm text-muted-foreground">
                Adjust your filters to see more analysis data.
              </p>
            </div>
          )}
